...
var PHILLY_OWNER_SPREADSHEET_ID = '11kIfdMNJ-6Pa-331AUViYpotnMRfM2sNoAuW7U0lLkA';

// 主要函數：處理費城餐廳確認表單提交並發送到 WordPress
function onPhillyOwnerFormSubmit(e) {
  try {
    Logger.log('=== 開始處理費城餐廳確認表單提交 ===');
    
    // 解析表單資料
    var formData = parsePhillyOwnerFormData();
    Logger.log('解析的費城餐廳確認資料:');
    Logger.log(JSON.stringify(formData, null, 2));
    
    // 檢查資料是否為空
    if (!formData || Object.keys(formData).length === 0) {
      throw new Error('解析的費城餐廳確認資料為空，請檢查試算表結構');
    }
    ...
    // 發送成功通知郵件
    sendPhillyOwnerNotificationEmail(formData, result);
    
    Logger.log('費城餐廳確認表單處理完成');
    
  } catch (error) {
    Logger.log('費城餐廳確認表單處理發生錯誤: ' + error.toString());
    sendPhillyOwnerErrorNotification(error);
  }
}

// 解析費城餐廳確認表單資料
function parsePhillyOwnerFormData() {
  try {
    Logger.log('=== 開始解析費城餐廳確認表單資料 ===');
    
    // 使用指定的試算表 ID
    var spreadsheet = SpreadsheetApp.openById(PHILLY_OWNER_SPREADSHEET_ID);
    ...
    Logger.log('費城餐廳確認表單標題行:');
    Logger.log(formHeaders);
    Logger.log('最新費城餐廳確認資料:');
    ...
    parsedData['source'] = 'philly_owner_verification'; // 修改來源標記
    ...
    Logger.log('解析費城餐廳確認表單資料時發生錯誤: ' + error.toString());
    throw error;
  }
}


// 發送到 WordPress（使用相同端點）
function sendToPhillyWordPress(data) {
  Logger.log('準備發送到 WordPress 的費城餐廳確認資料:');
  ...
  try {
    ...
    if (responseCode === 200 || responseCode === 201) {
      return {
        success: true,
        message: 'Successfully created Philadelphia BYOB restaurant post from owner verification',
...
// 發送成功通知郵件（餐廳業者版本）
function sendPhillyOwnerNotificationEmail(data, result) {
  try {
    var subject = 'Philadelphia BYOB Owner Verification - ' + (data.restaurant_name || 'Unknown Restaurant');
    var body = 
      '<h2>New Philadelphia BYOB Restaurant Owner Verification Received</h2>' +
      ...
      '<p><strong>Contact Name:</strong> ' + (data.philly_reddit_username || 'Not provided') + '</p>' +
      '<p><strong>Contact Email:</strong> ' + (data.philly_contact_email || 'Not provided') + '</p>' +
      ...
      '<hr>' +
      '<p><small>This email was sent by Philadelphia BYOB Owner Verification System</small></p>';
    
    GmailApp.sendEmail(NOTIFICATION_EMAIL, subject, '', {
      htmlBody: body
    });
    
    Logger.log('已發送費城餐廳確認通知郵件');
    
  } catch (error) {
    Logger.log('發送費城餐廳確認通知郵件時發生錯誤: ' + error.toString());
  }
}

// 發送錯誤通知郵件
function sendPhillyOwnerErrorNotification(error) {
  try {
    var subject = 'Philadelphia BYOB Owner Verification Error';
    var body = 
      '<h2>Philadelphia BYOB Owner Verification Error</h2>' +
...
// 測試函數：解析最新資料
function testParsePhillyOwnerData() {
  try {
    Logger.log('=== 測試解析費城餐廳確認資料 ===');
    var result = parsePhillyOwnerFormData();
    Logger.log('解析結果:');
    Logger.log(JSON.stringify(result, null, 2));
    return result;
  } catch (error) {
    Logger.log('測試解析費城餐廳確認資料時發生錯誤: ' + error.toString());
  }
}

// 測試函數：完整流程測試
function testPhillyOwnerCompleteFlow() {
  try {
    Logger.log('=== 測試費城餐廳確認完整流程 ===');
...
// 設定觸發器
function setupPhillyOwnerTrigger() {
  // 刪除現有觸發器
  var triggers = ScriptApp.getProjectTriggers();
  for (var t = 0; t < triggers.length; t++) {
    if (triggers[t].getHandlerFunction() === 'onPhillyOwnerFormSubmit') {
      ScriptApp.deleteTrigger(triggers[t]);
    }
  }
  
  // 建立新的表單提交觸發器
  ScriptApp.newTrigger('onPhillyOwnerFormSubmit')
    .forSpreadsheet(SpreadsheetApp.openById(PHILLY_OWNER_SPREADSHEET_ID))
    .onFormSubmit()
    .create();
    
  Logger.log('費城餐廳確認觸發器設定完成');
}

// 手動觸發測試（用於除錯）
function manualPhillyOwnerTest() {
  try {
    Logger.log('=== 手動測試費城餐廳確認流程 ===');
    
    // 模擬表單提交事件
    var mockEvent = {
      source: SpreadsheetApp.openById(PHILLY_OWNER_SPREADSHEET_ID),
      range: SpreadsheetApp.openById(PHILLY_OWNER_SPREADSHEET_ID).getSheets()[0].getDataRange()
    };
    
    onPhillyOwnerFormSubmit(mockEvent);
