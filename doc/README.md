...
### 今日完成：驗證徽章系統與 Yelp 連結整合

**🎯 關鍵成就**

* ✅ **驗證徽章系統實作**
  * 前台顯示：餐廳列表頁和單一餐廳頁新增驗證徽章
  * 兩種狀態：`Verified by Restaurant`（藍色🔒）、`Community Recommended`（橙色👥）
  * 後台管理：新增 `verification_override` 欄位，管理員可手動覆蓋驗證狀態

* ✅ **Yelp 連結欄位整合**
  * Google 表單更新：將「Website or Reservation Link」改為「Yelp Link」
  * 端到端整合：表單 → Apps Script → WordPress API → ACF → 前台顯示
  * 前台策略：單一餐廳頁顯示，列表頁不顯示

* ✅ **餐廳業者後台優化**
  * 加入 Yelp Link 欄位到餐廳業者編輯頁面
  * 欄位位置：在「Yelp Link / Official Website/Social Media Links」區塊的第一個位置
...
**🔧 技術架構**
- 驗證徽章系統：`byob_get_verification_badge_info()` 和 `byob_display_verification_badge()` 函數
- Yelp 整合：表單、後端 API、ACF 欄位、前台顯示完整流程
- 格式統一：餐廳列表頁和單一餐廳頁一致性優化
...
- **影響**：資料驗證困難、大量資料停滯待驗證
- **解決方案**：建立友善的 email 模板、設計多階段提醒機制、建立社群驗證備案（明日開始實作）

### 資料衝突處理 ⚠️
- **挑戰**：多名網友提供不同資訊，難以判斷正確性
- **影響**：資料準確性下降、用戶信任度降低
- **解決方案**：建立衝突檢測與解決機制、優先採用餐廳老闆驗證、保留多版本供人工審核（明日開始實作）
